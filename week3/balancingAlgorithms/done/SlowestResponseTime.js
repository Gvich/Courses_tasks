...
async function slowestResponseTime(banners) {
    ...
    let slowestResponseTime = -1; // Initialize with a negative value
    let selectedAd = null;

    // Find the ad with the slowest response time
    for (const response of responses) {
        if (response.responseTime > slowestResponseTime) {
...
async function simulateLoadBalancing() {
    ...
    const userPromises = Array.from({ length: userCount }, () => slowestResponseTime(ads));
